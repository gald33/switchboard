...
- **`one-resolved-context-across-surfaces`** — Decide whether a session may change its room once, for every surface at once  
  deferred: Filed on 2026-09-01 as a design question, not started, because it reverses a documented invariant and the thing it fixes has a working explicit answer. `docs/environments.md`: "The client reads all four from the environment and nowhere else — there is no config file, no `~/.switchboard/config`, no user-level scope." What looks per-repo is a runner injecting environment from committed files. A session-scoped store is exactly the tier that sentence rules out, so adding one is a decision about the model rather than a feature. Un-defer on evidence that the explicit route is failing in practice: an agent that demonstrably split itself across two rooms while trying to do the right thing, or a task where passing the room on every call proved unworkable.
...
> than about caching.
>
> If it were built as a store, it would be session-scoped and keyed by the
> harness session id that agent identity already derives from
> (`client.py:_SESSION_ID_VARS`), so concurrent sessions in one checkout do
> not share it. Resolution: flags > session state > environment > repo.
>

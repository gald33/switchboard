...
Everything else in Switchboard is built for an agent to read. This is for the
person the agents are working for — the one who currently has four terminals
open, or who asks "what are they even doing right now?" and has to answer it
with three subcommands and a mental merge.
...
**It never advances a cursor.** Messages come from `GET /channels/{c}`, the
catch-up read, rather than from `/inbox`. Watching a room therefore cannot
make an agent's next `inbox` come back empty. This is the one property worth
being paranoid about: the failure it prevents is silent on both sides — a
message an agent never sees, and a human who watched it go past.

There is no plan to add a "reply" box. Coordination that matters is between
agents, and a human typing into the room is better served by the CLI, where
what was said and who said it are unambiguous.
...
[End-to-end encryption](encryption.md). The viewer is a local process holding
the same configuration your agents hold, and it decrypts in the only place
that can.
...
It resolves its hub, workspace, token and key exactly like every other CLI
command, so it shows the room the agents in this directory are actually in.
If your agents are in a room you have to `export` your way into, export the
same things before starting it.

## Embedding it

The two pieces are public:

```python
from switchboard import Client
from switchboard.web import snapshot, make_server

with Client() as hub:
    view = snapshot(hub)          # one JSON-able dict: agents, leases, board,
    print(view["messages"])       # channels, messages, and notes about what
                                  # could not be read
```
...
The page itself is `src/switchboard/viewer.html`: one file, no build step, no
CDN, and re-read on each request so editing it is a browser refresh.
